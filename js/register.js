document.addEventListener('DOMContentLoaded', function() {
  ...
  if(passwordInput && strengthBar && strengthLabel){
    passwordInput.addEventListener('input', function(){
      ...
      if(s <= 1) strengthBar.style.backgroundColor = '#ef4444';
      else if(s <= 3) strengthBar.style.backgroundColor = '#f59e0b';
      ...
      if(pw.length === 0){
        ...
        strengthLabel.textContent = 'Medium - could be stronger';
    ...
    googleRegister.addEventListener('click', function(){
      ...
      modal.innerHTML = `
        <div style="background:#fff;border-radius:16px;padding:32px;max-width:400px;width:90%;text-align:center;box-shadow:0 16px 48px rgba(0,0,0,0.2)">
          ...
          <h3 style="margin:0 0 8px;font-size:20px;font-weight:700">Create Account with Google</h3>
          <p style="color:#6b7280;margin:0 0 20px;font-size:14px">Enter your details to create account</p>
          ...
          <div style="display:flex;gap:12px">
            ...
            <button id="demo-google-submit" style="flex:1;padding:14px;border:none;background:linear-gradient(135deg,#ff6b01,#ff8c3a);color:#fff;border-radius:10px;cursor:pointer;font-weight:600">Create Account</button>
          </div>
        </div>
      `;
      ...
      submitBtn.onclick = function(){
        ...
        const msg = document.createElement('div');
        msg.style.cssText = 'position:fixed;top:20px;left:50%;transform:translateX(-50%);background:#10b981;color:#fff;padding:14px 28px;border-radius:8px;z-index:9999;font-weight:600';
